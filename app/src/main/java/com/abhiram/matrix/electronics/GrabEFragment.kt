...
import com.abhiram.matrix.adapter.EItem
import com.abhiram.matrix.adapter.EItemAdapter
...
class GrabEFragment : Fragment() {
    private var list : ArrayList<EItem> = ArrayList()
    ...
    override fun onCreateView(
        ...
        val inflate = inflater.inflate(R.layout.fragment_grab_e, container, false)
        read()
        ...
        val adapter = EItemAdapter(list,inflate, sharedPreferences,requireContext())
    ...
    fun read(){
        ...
        val sharedPreferences = SharedPreferncesHelper()
        sharedPreferences.SharedPreferncesHelperInit(requireContext())
        cat = sharedPreferences.getECat()
        val path = "Electronics/$cat"
        Log.e("Cta",cat)
        val gson : Gson = Gson()
        ...
        mail.addValueEventListener(object: ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                for (i in snapshot.children){
                    ...
                    val temp : EItem? = gson.fromJson(json, EItem::class.java)
