...
class GiveVFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_give_v, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val modelReg = database.getReference("Vehicles/$cat/Give/item${count+1}/VehicleModel")
        val nameReg = database.getReference("Vehicles/$cat/Give/item${count+1}/Name")
        val regNo = database.getReference("Vehicles/$cat/Give/item${count+1}/Regno")
        val periodReg = database.getReference("Vehicles/$cat/Give/item${count+1}/Period")
        val uidReg = database.getReference("Vehicles/$cat/Give/item${count+1}/Uid")
        val descpn = database.getReference("Vehicles/$cat/Give/item${count+1}/Description")
        val isTaken = database.getReference("Vehicles/$cat/Give/item${count+1}/IsTaken")
        ...
        submit.setOnClickListener {
            ...
            if (desp.trim().isNotEmpty()&&
                ...
                Toast.makeText(requireContext(),"Your Vehicle has been listed for sharing", Toast.LENGTH_SHORT).show()
            }else{
